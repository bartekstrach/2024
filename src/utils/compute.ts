...
const mapScoreboardToArray = (
  scores: Map<string, number>,
): Array<Scoreboard> => {
  return Array.from(scores).map(([participant, points]) => ({
    participant,
    points,
  }));
};

const sortByPoints = (scoreboard: Array<Scoreboard>): Array<Scoreboard> =>
  [...scoreboard].sort((s1, s2) => s2.points - s1.points);
